...
def main(overwrite=0):
    ...
    ###########################
    # 25A/B / Cycle7 specific #  (note: 25B: only really have info to mid-Sept)
    ###########################
    df['c7observable'] = np.array(max_sector) >= 84
    #########################

    viable_sectors = range(95, 96+1)
    is_25b_observables = []
    for sstr in df['sectors'].astype(str):
        is_25b_observable = False
        for s in viable_sectors:
            if str(s) in sstr:
                is_25b_observable = True
                continue
        is_25b_observables.append(is_25b_observable)

    df['is_25b_observable'] = np.array(is_25b_observables).astype(int)

    selcols = [
        'ticid', 'sectors', 'c7observable', 'is_25b_observable', 'N_sectors', 'N_200sec', 'N_FFI',
    ...
    # OBSERVABLE IN 25b, SIMULTANEOUS WITH TESS
    outcsvpath = join(
        indir,
        '20240304_CPV_lit_compilation_R16_S17_S18_B20_S21_Z19_G22_P23_B24_TIC8_obs_truncated_25bOBSERVEABLE.csv'
    )
    sdf[sdf.is_25b_observable == 1].sort_values(by='tic8_Tmag').to_csv(
