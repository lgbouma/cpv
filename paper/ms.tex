...
\begin{abstract}
  ...
  smooth, spot-like modulation.  These objects are typically
  10-100 million years (Myr) old, and they do not seem to have primordial gas-rich
  ...
  \ngoods\ gold-standard CQVs.  Most of these discoveries are new,
  and they include the brightest and closest
  examples of this class of object known ($T$$\approx$10; $d$$\approx$20\,pc).  A few
  objects are outliers among outliers; LP 12-502 for instance
  shows four to eight local minima per cycle distributed over a ``dip
  complex'' with a total duration that is nearly fixed over more than
  1000 cycles.  Nonetheless, LP-502 at times also showed drastic shape
  changes over less than one cycle.  More generally, we find that none
  of the CQVs maintain a fixed light curve shape over timescales of
  more than a few hundred cycles -- they are {\it quasi}periodic.
  Although the physical explanation behind these objects remains
  enigmatic, our sample may help clarify their origin.  Connections to
  star, disk, and exoplanet evolution are emphasized.
\end{abstract}
...
the stellar surface \citep[e.g.][]{2017AJ....153..152S,2022AJ....163..144G}.  In the ``clump''
...
\citep{2016AJ....152..114R,2017AJ....153..152S}, and automatically flagging rapid rotators
...
pipeline were 5:1, with \nuniqdipflagged\ unique stars flagged, and about 20\% being
...
kinematic associations between the CQVs and known clusters in the solar
neighborhood.  For most of the stars in our sample, we were able to do
this using BANYAN\,$\Sigma$
\citep{2018ApJ...856...23G}.\footnote{\url{https://github.com/jgagneastro/banyan_sigma},
git commit \texttt{394b486}} This algorithm calculates the
probability that a given star belongs to one of 27 young
clusters (or ``assocations'') within 150\,pc of the Sun, by modeling the clusters
as multivariate Gaussians in 3-D position and 3-D velocity space.  
We used the
Gaia DR2 sky positions, proper motions, and distances to calculate the
membership probabilities.  
BANYAN\,$\Sigma$ in turn analytically marginalizes over the radial velocity dimension.
We emphasize that the probabilities returned by this procedure are qualitatively
useful, but quantitatively dubious due to the non-Gaussian
nature of most groups within the
solar neighborhood \citep[see e.g.][Figure~10]{2021ApJ...917...23K}.

For a few cases where BANYAN\,$\Sigma$
yielded ambiguous results, we consulted the meta-catalog of young,
age-dated, and age-dateable stars within a kiloparsec from
\citet{2022AJ....163..121B}, and also searched the local volume around
each star for co-moving
companions.\footnote{\url{https://github.com/adamkraus/Comove}, git commit \texttt{278b372}}
...
characteristics of the CPV phenomenon in at least one TESS sector, and
we refer to these as the ``gold sample''.
The remaining \nmaybes\ CPV candidates were ambiguous (in our view),
and their variability might be caused by say, multiple starspot
groups.

The mosaic in Figure~\ref{fig:singlemosaic} shows phased light curves
for the 40 brightest objects.
The TESS sectors in which these objects were identified show
...
were confidently associated with a nearby moving group or open
cluster using BANYAN\,$\Sigma$.
The relevant groups are listed in Table~\ref{tab:thetable}; their ages
span $\approx$5-150\,Myr.
The most prodigious groups were Sco-Cen, Tuc-Hor, and Columba.
The yield of CQVs in Sco-Cen (LCC / UCL / USco) is not surprising,
since Sco-Cen contains the majority of pre-main-sequence stars in the
solar neighborhood.
The absence of CQVs in IC2602 and IC2391 is because those clusters are
beyond 100\,pc, where the completeness of the TESS 120-second cadence
data is below 10\%.

Of the the \ngoodsfieldbanyan\ non-associated stars, one
(TIC~302160226) is an obvious member of $\alpha$~Per ($t\approx
86\pm16$\,Myr; \citealt{2022arXiv221109822B}).  For the other two
...
either star with any young groups.
Both were noted as being in the ``diffuse'' Class~II YSO
population by \citet{2021ApJ...917...23K}.
...
stars within $\Delta r$=$40$\,pc and $\Delta v$=$3$\,km\,s$^{-1}$
of these stars suggest that they may have run away from their birth
locations.
...
unambiguous case (EPIC~211070495) and a few other candidates were
also previously known in the Pleiades \citep{2016AJ....152..114R}.
While we expect to have been sensitive to CQVs in the nearby Hyades
($\approx$700\,Myr), 
again due to the specifics of the TESS 120-second selection function,
we do not know at this time whether we would have been sensitive to
such stars.

Finally, six CQVs were identified in the recently
confirmed Argus association \citep{2019ApJ...870...27Z}.
This serves as an indirect line of evidence supporting the reality,
and youth, of that group.
...
\paragraph{LP 12-502}
While many CPVs had multiple sectors of data, in our view the most
astounding dataset was available for LP 12-502 (TIP 402980664).
Figure~\ref{fig:lp} shows all available data from Sectors 18, 19, 25,
26, 53, 58, and 59; the star was observed at 120-second cadence for
every sector during which it was on silicon.

For the first 64 cycles, the star shows a pattern reminiscent of a
(bizarre) eclipsing binary, with four obvious dips, that we denote
dips 1, 2, 3, and 4 at phases -0.28, -0.08, 0, and 0.25 respectively.
The depth of dips 1 and 3 remain roughly fixed.
Over the 64 cycles, dip 4 {\it decreases} in depth by about 2\%, while
dip 2 {\it increases} in depth, by about the same amount.

There is then a 6-month (184 cycle) gap out to Cycle 248.
At this time, dip 1 is still present, at the same phase, and with
roughly the same depth.

%TODO continue writing about this.  What is the exact TJD of the jump?

%TODO; make a version (for talks) showing dips 1234 over cycles0-64
%TODO include +/- 1/3 as important durations.
%TODO: note that duration between the minimum of "dip1" and "dip4" is
%      kind of always 0.53
...
\begin{figure*}[!t]
    ...
    \includegraphics[width=0.9\textwidth]{TIC_402980664_P18.5604_2min_phase_timegroups_mosaic.pdf}
		...
		\caption{
			{\it Bottom:}
      Evolution of LP 12-502 ($P$=18.5604\,h) at fixed period and epoch over three
...
L.G.B.~performed the cluster membership analysis.
