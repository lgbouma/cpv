...
Data from K2 and TESS have yielded a new class of variable star---
complex quasiperiodic variables---for which the underlying cause of
variability has only begun to be explored recently. These objects are 
identified phenomenologically
using their optical light curves, which show nearly periodic troughs
that are either sharp or broad; these are often superposed on smooth
spot-like modulations
...
We note that the ``proper'' nomenclature for these stars in the literature
has generally been unclear. CQVs have been called ``transient flux
...
sectors also include full frame images with cadences of 600 and 1800 seconds,
we limited our scope to 120-s cadence in this work for the sake of simplicity in data
...
For the Fourier analysis, we followed the techniques presented in 
\citet{2019ApJ...876..127Z} and Section 1.3 of \citet{2023MNRAS.524.4220P}.
All 120-s targets were searched for strongly periodic phenomena using our
Fourier pipeline. We used the {\tt PDCSAP\_FLUX} column from the data  
and normalized and  re-binned the light curve into equal width bins, due to the
uneven spacing in the TESS data, as well as the data gap in the middle of
each sector due to the lack of data collection during downlink. Data was
padded to ensure that the light curve had a length that was a power of 2, 
as described in \citeauthor{2019ApJ...876..127Z}. We
...
If a peak of such significance was found, we generated a ``summary sheet'' 
with information about the target, its full and folded light curves, Fourier 
transform, potential contaminating stars, and information about these
 contaminating stars. Throughout the 35 sectors for which we performed
 this analysis, we found that ``summary'' sheets were generated for 
 approximately 10\% of the 20\,000
120-s targets in a given sector. Each summary sheet was manually reviewed.
During this manual review, each light curve was sorted into a category based on its
morphology (e.g., eclipsing binary, CQV, or cataclysmic variable). For
the CQVs, a further cut was made based on the target's temperature---any
stars exhibiting phenomena that looked similar to CQVs, but had a $T_{\rm eff}$ 
over 4\,000\,K were discarded from the target list (e.g., RS CVn stars). 
{\red (RJ: can we add an example of these sheets to this paper? i know it's not 
the ``main'' vetting, though, so happy to not do this as well)}.

CQVs that were observed in multiple sectors had their light curves
stitched together and analyzed for a more accurate period determination.
...
The Fourier pipeline, on the other hand, was less CQV-focused than
the dip-counting technique, as its results were used for other sorts of
analyses; as a result, we cannot calculate a similar ``false-positive''
probability. Between the targets identified as CQVs by both pipelines,
