...
\nallcands\ star sample, we labeled two objects as having ``good''
infrared excesses (both W3 and W4 at $>$3$\sigma$ above the
photospheric prediction), and three as ``possible'' infrared excess
(only W3 at $>$3$\sigma$).

The two ``good'' IR excesses belonged to TIC~193136669 (TWA~34) and
...
\begin{figure*}[!tp]
	\begin{center}
		...
		\caption{
			...
			each point is connected by a line.  Data gaps longer than
			15-minutes are not interpolated; if data are missing, nothing is
			plotted.
		}
		\label{fig:lplc}
	\end{center}
