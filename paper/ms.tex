...
An interesting point of comparison is the rapidly rotating magnetic
...
of absorbing circumstellar material, analogous to the geometries we
are discussing for much cooler M-dwarfs \citep{2005ApJ...630L..81T}.
This material however is unlikely to be dust, due to the relevant
sublimation timescales.  The opacity is instead thought to be sourced
from bound-free absorption from neutral hydrogen
\citep{2005ApJ...630L..81T}.  Separate and smaller-amplitude emission
in that system may also come from electrons scattering photospheric
light toward the observer when the clouds are not in transit
...
\begin{figure*}[!t]
	\begin{center}
		...
		\caption{
			...
			HD~37776 (TIC~11400909) and HD~64740 (TIC~268971806) are
			magnetic B-stars with field geometries dominated by
			high order multipoles.
			Here, we compare their phased TESS light curves against the
			``best-matching'' CQV light curves, selected by eye from
			Figure~\ref{fig:cqvs}.
			Stellar masses approximate to one significant figure are given in the
			lower right of each panel; the star, TESS sector, and period are also noted.
		}
		\label{fig:bstar}
	\end{center}
\end{figure*}
...
Previous work by \citet{2017AJ....153..152S} noted an intriguing
connection between the CQVs and rapidly rotating magnetic B-stars such
as $\sigma$~Ori~E, which are known to have circumstellar gas clouds
trapped in corotation \citep{2005ApJ...630L..81T}.  Spectropolarimetry
has indicated that, similar to $\sigma$~Ori~E, most magnetic B-stars
have relatively simple field geometries that are predominantly
dipolar, though generally not aligned with the stellar spin axis
\citep{2009ARA&A..47..333D}.  Two exceptions to this rule are HD~37776
and HD~64740, which are B-stars that were previously known from
spectropolarimetry in the 2000s-2010s to have more complicated field
geometries - dominated by high order multipoles
\citep{2011ApJ...726...24K}.  It turns out, recent TESS light curves
of these two B-stars look surprisingly similar to the CQV light curves
\citep{2020pase.conf...46M}.  
Figure~\ref{fig:bstar} shows the phased TESS light curves for
the two outlying magnetic B-stars,
with by-eye ``best matching'' CQVs shown for comparison.
The number of dips per cycle, the shapes of the dips, and the dip
depths relative to the sinusoidal envelope, all seem to be quite
similar.
If this connection is taken seriously, then the implication would be
that the highly-structured M-dwarf light curves imply multipolar magnetic fields --
since that is the defining characteristic of these two B-stars,
relative to other known magnetic B stars like $\sigma$~Ori~E.  
