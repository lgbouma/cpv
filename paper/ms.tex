...
Figures 37-41).  The observed dips can happen over durations as
...
for the time being, ``CQV'' seems to more accurately mirror the state
of our ignorance.
...
12-$\sigma$ within a set of 500 bins. 
{\bf LB: by the latter ``bins'', does this mean frequencies?}
...
WISE 1-2 passbands.  We omitted UV flux measurements from
...
pre-main-sequence stars in the solar neighborhood, any many of its stars
...
This could be resolved by acquiring even a
...
The two IR excess sources were TIC~193136669 (TWA~34) and TIC~57830249
...
\nrvscatter\ stars: TIC~405910546, TIC~224283342, and TIC~280945693.
%
...
``non\_single\_star'' column that flagged known or suspected
...
section, we first present the LP 12-502 observations
(Section~\ref{subsec:lpobservations}), and then highlight their
implications (Section~\ref{subsec:lplessons}).
...
Thompson scattering is similarly ruled out as a relevant opacity
...
be a product of the human tendency to pattern match.  The alternative
is that some subset of these pattern could  have real physical
significance.
...
\citealt{2022ApJ...924L..10J}).  Within the model proposed by Townsend
et al., this is explained by the simplicity of the star's magnetic
field, which to first-order is a dipole, similar to most other
