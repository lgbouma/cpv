...
% 250 word max!
\begin{abstract}
%  This work focuses on a subset of low-mass pre-main-sequence stars
%  with rotation periods of 0.2--2 days, optical variability amplitudes
...
%  We therefore refer to the objects as ``transient corotating clumps''
%  (TCCs).
%
  Complex rotators (CRs) are low-mass pre-main-sequence stars with light
  curves characterized by periods of 0.2--2 days, amplitudes of a few
  percent, and slowly-evolving sharp features.  The sharp features
  might be caused by eclipsing clumps of dust or gas orbiting at the
  corotation radius.
  To improve our understanding of these
  enigmatic stars, we conducted a survey for CRs using TESS
  ...
  correspondences between the light curves of CRs and of two B stars
...
indistinguishable; one could imagine dust or gas accumulating in
...
concentrates.  This is empirically justified by the observation that
...
radius has been discussed in studies of rapid magnetic rotators,
including neutron stars \citep[e.g.]{1973ApJ...184..271L}, magnetic
massive stars \citep[e.g.][]{1985Ap&SS.116..285N}, and accreting T
Tauri stars \citep[e.g.][]{2005ApJ...634.1214L}.  In regions where the
magnetic field dominates the flow (i.e.  $B^2/8\pi > \rho v^2 /2$),
matter is dragged along with the field lines.  Assuming a dipolar
magnetic field geometry, then the tendency of both gas and dust to
become trapped at corotation is tied to how of the four relevant
forces -- gravity, Lorentz, inertial Coriolis, and inertial
centrifugal -- the Lorentz and Coriolis only act perpendicular to
field lines, while gravity and the centrifugal force are in balance at
$R_{\rm c}$ \citep[e.g.][Sec.~2]{2005MNRAS.357..251T}.  Another way to
phrase this statement is that there is a local minimum in the
effective potential of the corotating frame; given a flow from the
either the star or from a tenuous accretion disk, this local potential
minimum enables material to build up \citep{2005MNRAS.357..251T}.

While theoretical heritage for understanding rapid magnetic
...
\begin{figure}[!tp]
	\begin{center}
		\centering
		\includegraphics[width=0.48\textwidth]{f2completeness.pdf}
		\vspace{-0.5cm}
		\caption{
...
not see any such instances of dips switching on.  Dip growth instead
seems to happen more slowly.  For instance, the dip at phase 0-0.1
between cycles 258-290 begins to become detectable during cycle 258,
and growths in depth by about 2\% over the next eight cycles.  The
...
1.08\,hr.  This is very similar to the characteristic timescale of of
...
theoretically plausible for such stars
...
disk's truncation radius \citep{2018haex.bookE.142I}.  The disk
...
drag, and the innermost planets arriving at $\approx$5-10 stellar
...
R_{\rm c}$, for $R_{\rm M}$ the magnetospheric radius (sometimes
...
``dynamical'' case, when  $R_{\rm M} < R_{\rm c}$, material
...
times the corotation radii, $R_{\rm c}$.  This suggests that the
...
can be accessed via
\dataset[DOI]{https://doi.org/10.17909/t9-nmc8-f686}.  Funding for the
...
\begin{figure*}[!t]
	\begin{center}
	\subfloat{
			\includegraphics[width=0.9\textwidth]{f13a.pdf}
    }

	\vspace{-1.1cm}
	\subfloat{
			\includegraphics[width=0.9\textwidth]{f13b.pdf}
    }

	\vspace{-1.1cm}
	\subfloat{
			\includegraphics[width=0.9\textwidth]{f13c.pdf}
    }
	\end{center}
	\vspace{-0.4cm}
	\caption{
